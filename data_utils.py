...
class Constants:
    def __init__(self, sample_rate=16000, analysis_samples=8000, ticks_per_quarter=960):
                        ...
                        for beat in beats:
                            if beat.effect.mixTableChange:
                                ...
                                if beat.status.name != 'normal':
                                    ...
                                    note_event = {}
                                    note_event['beat_duration'] = beat.duration.time/constants.ticks_per_quarter
                                    note_event['beat_onset_piece'] = beat.start/constants.ticks_per_quarter
                                    note_event['beat_onset_measure'] = beat.startInMeasure/constants.ticks_per_quarter
                                    note_event['secs_duration'] = note_event['beat_duration']*60/self.tempo
                                    note_event['secs_onset_piece'] = note_event['beat_onset_piece']*60/self.tempo
                                    note_event['secs_onset_measure'] = note_event['beat_onset_measure']*60/self.tempo
                                    note_event['samples_duration'] = np.floor(note_event['secs_duration']*constants.sample_rate).astype(int)
                                    note_event['samples_onset_piece'] = np.floor(note_event['secs_onset_piece']*constants.sample_rate).astype(int)
                                    note_event['samples_onset_measure'] = np.floor(note_event['secs_onset_measure']*constants.sample_rate).astype(int)
                                    note_event['pitches'] = []
                                    ...
                                    for n in beat.notes:
                                        if n.type.value == 1:
                                            ...
                                            pitch_event['midi_velocity'] = n.velocity
                                            pitch_event['velocity_ratio'] = n.velocity/127
                                            pitch_event['duration_percentage'] = n.durationPercent
                                            pitch_event['secs_duration_percentage'] = n.durationPercent*note_event['secs_duration']
