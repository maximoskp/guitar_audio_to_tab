...
BPM-free frame-level tablature + onset + chord-aware event assembly training.

This is a new set of scripts for the "event assembly head" idea. It keeps the
Tab-Estimator-Hand-Onset frame/onset core, then adds a learned temporal head that
predicts:
  - event existence
  - selected strings
...
The event head is trained from the same frame_tab and frame_onset targets. It is
non-causal and uses a TCN with at least 4 layers.
"""
...
def derive_event_type_targets(
    frame_onset,
    chord_context_frames=1,
    arpeggio_context_frames=5,
):
    """
    Derive event type labels:
      0 = no event
      1 = single isolated note
      2 = chord-like event
      3 = arpeggio-context event

    Chord means: multiple distinct strings have onsets within a tight local
    window. Arpeggio means: the current frame has an onset and other strings
    have onsets in a wider context, but not in the chord window.
    """
    onset = np.asarray(frame_onset, dtype=np.float32) > 0
    if onset.ndim != 2 or onset.shape[1] != 6:
        raise ValueError(f"Expected frame_onset shape (T,6), got {onset.shape}")

    T = onset.shape[0]
    out = np.zeros((T,), dtype=np.int64)

    chord_context_frames = max(0, int(chord_context_frames))
    arpeggio_context_frames = max(chord_context_frames + 1, int(arpeggio_context_frames))

    for t in range(T):
        if not onset[t].any():
            continue

        c0 = max(0, t - chord_context_frames)
        c1 = min(T, t + chord_context_frames + 1)
        chord_strings = np.where(onset[c0:c1].any(axis=0))[0]

        if len(chord_strings) >= 2:
            out[t] = EVENT_TYPE_CHORD
            continue

        a0 = max(0, t - arpeggio_context_frames)
        a1 = min(T, t + arpeggio_context_frames + 1)
        arp_strings = np.where(onset[a0:a1].any(axis=0))[0]

        if len(arp_strings) >= 2:
            out[t] = EVENT_TYPE_ARPEGGIO
        else:
            out[t] = EVENT_TYPE_SINGLE
...
class FrameOnsetEventDataset(Dataset):
    def __init__(
        self,
        data_list,
        input_feature_type,
        ...
        event_chord_context_frames=1,
        event_arpeggio_context_frames=5,
    ):
        ...
        self.event_chord_context_frames = int(event_chord_context_frames)
        self.event_arpeggio_context_frames = int(event_arpeggio_context_frames)
    ...
    def __getitem__(self, index):
        ...
        frame_onset_hard = load_frame_onset_from_npz(data, frame_tab).astype(np.float32)

        frame_len = int(input_features.shape[0])
        target_len = min(frame_len, frame_tab.shape[0], frame_onset_hard.shape[0])

        input_features = input_features[:target_len]
        frame_tab = frame_tab[:target_len]
        frame_onset_hard = frame_onset_hard[:target_len]

        event_type = derive_event_type_targets(
            frame_onset_hard,
            chord_context_frames=self.event_chord_context_frames,
            arpeggio_context_frames=self.event_arpeggio_context_frames,
        )
        ...
        return input_features, frame_tab, frame_onset, event_type, frame_len, frame_hand_pos


def _pad_1d(x, target_len):
...
def frame_onset_event_collate(batch):
    input_features, frame_tab, frame_onset, event_type, frame_len, frame_hand_pos = zip(*batch)
    ...
    event_type_out = np.asarray([_pad_1d(x, max_len) for x in event_type], dtype=np.int64)
    ...
    event_type_out = np.take(event_type_out, sort_idx, axis=0)
    ...
    return (
        ...
        torch.from_numpy(event_type_out),
...
def build_tab_estimator(
    ...
    return TabEstimator(
        ...
        onset_tcn_levels=max(4, int(onset_tcn_levels)),
        ...
        event_head_tcn_levels=max(4, int(event_head_tcn_levels)),
...
def run_epoch(model, loader, criterion, optimizer, device, train_mode=True, freeze_encoder=False, freeze_frontend=False):
    ...
    for batch in loader:
        input_features, frame_tab, frame_onset, event_type, frame_len, frame_hand_pos = batch
...
def train(
    ...
    model = build_tab_estimator(
        ...
        onset_input_mode=args.onset_input_mode,
        event_head_hidden_dim=args.event_head_hidden_dim,
        event_head_tcn_levels=args.event_head_tcn_levels,
        event_head_kernel_size=args.event_head_kernel_size,
        event_head_dropout=args.event_head_dropout,
        event_input_uses_tab_probs=not bool(args.no_event_tab_probs),
        detach_tab_probs_for_event=bool(args.detach_tab_probs_for_event),
    )
    ...
    criterion = CustomLoss(
        ...
        event_string_loss_weight=args.event_string_loss_weight,
        event_string_positive_weight=args.event_string_positive_weight,
        event_fret_loss_weight=args.event_fret_loss_weight,
    ...
    chord_context_frames = ms_to_frames(args.event_chord_context_ms, hop_length=hop_length, sample_rate=sr)
    arpeggio_context_frames = ms_to_frames(args.event_arpeggio_context_ms, hop_length=hop_length, sample_rate=sr)

    train_dataset = FrameOnsetEventDataset(
        train_data_list,
        input_feature_type=input_feature_type,
        use_hand_position=use_hand_position,
        hand_pos_dim=hand_pos_dim,
        ...
        onset_target_sigma_frames=max(1.0, onset_radius_frames / 2.0),
        event_chord_context_frames=chord_context_frames,
        event_arpeggio_context_frames=arpeggio_context_frames,
    )
    valid_dataset = FrameOnsetEventDataset(
        valid_data_list,
        input_feature_type=input_feature_type,
        use_hand_position=use_hand_position,
        hand_pos_dim=hand_pos_dim,
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        shuffle=True,
        collate_fn=frame_onset_event_collate,
...
def parse_args():
    parser = argparse.ArgumentParser(description="BPM-free frame-tab + onset + chord-aware event assembly training")
    ...
    parser.add_argument("--onset-tcn-levels", type=int, default=4, help="Number of non-causal dilated gated TCN blocks. Must be >=4.")
    parser.add_argument("--onset-target-radius-ms", type=float, default=25.0)
    parser.add_argument("--onset-target-mode", choices=["hard", "triangular", "gaussian"], default="hard")
    ...
    parser.add_argument("--onset-input-mode", choices=["full", "encoder", "raw"], default="full")

    parser.add_argument("--event-loss-weight", type=float, default=0.50)
    parser.add_argument("--event-positive-weight", type=float, default=10.0)
    parser.add_argument("--event-string-loss-weight", type=float, default=0.50)
    parser.add_argument("--event-string-positive-weight", type=float, default=10.0)
    parser.add_argument("--event-fret-loss-weight", type=float, default=1.0)
    parser.add_argument("--event-type-loss-weight", type=float, default=0.10)
    parser.add_argument("--event-head-hidden-dim", type=int, default=128)
    parser.add_argument("--event-head-tcn-levels", type=int, default=6, help="Non-causal event assembly TCN blocks. Must be >=4; default 6.")
    parser.add_argument("--event-head-kernel-size", type=int, default=5, help="Odd kernel size for event assembly TCN.")
    parser.add_argument("--event-head-dropout", type=float, default=0.25)
    parser.add_argument("--event-chord-context-ms", type=float, default=35.0)
    parser.add_argument("--event-arpeggio-context-ms", type=float, default=150.0)
    parser.add_argument("--no-event-tab-probs", action="store_true", help="Do not feed flattened frame-tab probabilities to event head.")
    parser.add_argument("--detach-tab-probs-for-event", action="store_true", help="Stop event loss gradients from flowing through frame-tab probabilities.")
...
def main():
    ...
    metadata = {
        "architecture": "bpm_free_frame_tab_onset_chord_aware_event_assembly",
        "npz_dir": npz_dir,
        ...
        "onset_tcn_levels": int(max(4, args.onset_tcn_levels)),
        "event_head": "chord_aware_noncausal_gated_tcn",
        "event_head_hidden_dim": int(args.event_head_hidden_dim),
        "event_head_tcn_levels": int(max(4, args.event_head_tcn_levels)),
        "event_head_kernel_size": int(args.event_head_kernel_size),
        "event_head_dropout": float(args.event_head_dropout),
        "event_chord_context_ms": float(args.event_chord_context_ms),
        "event_arpeggio_context_ms": float(args.event_arpeggio_context_ms),
        "event_loss_weight": float(args.event_loss_weight),
        "event_string_loss_weight": float(args.event_string_loss_weight),
        "event_fret_loss_weight": float(args.event_fret_loss_weight),
        "event_type_loss_weight": float(args.event_type_loss_weight),
        "event_input_uses_tab_probs": not bool(args.no_event_tab_probs),
        "detach_tab_probs_for_event": bool(args.detach_tab_probs_for_event),
        "args": vars(args),
    }
    ...
    print("event head TCN levels:", max(4, args.event_head_tcn_levels))
