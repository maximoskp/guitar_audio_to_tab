...
Audio-only onset detection training script.

This script trains an audio-only onset detector without hand-position input
or tablature prediction. It focuses solely on predicting note onsets.

Training targets:
  - frame_onset: (T, 6), binary onset label per string per frame

The onset branch has two non-causal gated TCN heads: a per-string onset head and a
global/no-string onset head. Both receive encoder states plus a projected copy of
the raw CQT/mel features by default. They return raw logits and are trained with
BCEWithLogitsLoss inside OnsetOnlyLoss.
...
to a played fret. Training labels can be widened in time, e.g. +/-25 ms, while
evaluation still uses original onset locations.
"""
...
from network import OnsetDetector, OnsetOnlyLoss


# Fallback REST_CLASS for derive_frame_onsets_from_frame_tab if needed
REST_CLASS = 20
...
def load_frame_onset_from_npz(data, frame_tab=None):
    """
    Load frame onset targets from NPZ file.

    Priority order:
    1. Existing onset keys: frame_onset, frame_onsets, onset, onsets, frame_tab_onset
    2. If no onset key but frame_tab exists, derive from frame_tab
    ...
    raise KeyError(
        "NPZ does not contain onset targets (frame_onset, frame_onsets, onset, onsets, frame_tab_onset) "
        "and cannot derive them (frame_tab not provided). "
        "Please provide a suitable onset target or frame_tab."
    )
...
class OnsetDataset(Dataset):
    """
    Audio-only onset detection dataset.

    Loads input features and onset targets. No hand position or tablature input.
    """
    ...
    def __getitem__(self, index):
        ...
        # Try to load frame_onset; fall back to deriving from frame_tab
        frame_tab = data.get("frame_tab", None)
        if frame_tab is not None:
        ...
        return input_features, frame_onset, frame_len
...
def onset_collate(batch):
    """Collate function for OnsetDataset."""
    input_features, frame_onset, frame_len = zip(*batch)
...
def model_forward(model, padded_input_features, frame_len):
    return model(
...
def run_epoch(model, loader, criterion, optimizer, device, train_mode=True, freeze_encoder=False, freeze_frontend=False):
    ...
    for batch in loader:
        input_features, frame_onset, frame_len = batch
        ...
        with torch.set_grad_enabled(train_mode):
            frame_onset_pred, global_onset_pred, olens = model_forward(
                model,
                input_features,
                frame_len,
            )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Audio-only onset detection training")
    ...
    parser.add_argument("--all-folds", action="store_true", help="Train all folds (equivalent to omitting --test-num).")
    ...
    parser.add_argument("--onset-input-mode", choices=["full", "encoder", "raw"], default="full", help="Onset input mode: full (encoder+raw), encoder (encoder only), or raw (raw features only).")
...
def main():
    ...
    note_resolution = int(config.get("note_resolution", 4))
    train_ratio = float(config.get("train_ratio", 0.8))
    seed_value = int(args.seed if args.seed is not None else config.get("seed_", 0))
    ...
    metadata = {
        "architecture": "audio_onset_detection",
        "npz_dir": npz_dir,
        ...
        "uses_hand_position": False,
        "outputs": ["frame_onset_pred", "global_onset_pred"],
