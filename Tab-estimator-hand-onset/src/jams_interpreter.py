...
def jams_to_midi(jam, tempo=120, q=1, quantization=0):
    """
    Convert GuitarSet/DadaSet JAMS note_midi annotations to PrettyMIDI.

    q = 1: with pitch bend
    q = 0: without pitch bend

    quantization:
        0  -> do not quantize
        >0 -> quantize to note grid, e.g. 16 for 16th notes

    This version avoids the original while-loop quantizer, which can hang
    when symbolic DadaSet note times land exactly on grid boundaries.
    """
    ...
    if quantization > 0:
        note_dur = 60.0 / float(tempo) / float(quantization) * 4.0
    elif quantization < 0:
        sys.exit("Quantization parameter must be 0 or integer greater than 1")
    else:
        note_dur = None

    # Tab-Estimator expects six strings max.
    for i, anno in enumerate(annos[:6]):
        midi_ch = pretty_midi.Instrument(program=25, name=string_name[i])

        for note in anno:
            pitch = int(round(float(note.value)))
            bend_amount = int(round((float(note.value) - pitch) * 4096))

            if quantization != 0:
                # Safe quantization: no while loops.
                st = round(float(note.time) / note_dur) * note_dur

                dur_steps = int(round(float(note.duration) / note_dur))
                dur_steps = max(1, dur_steps)

                dur = dur_steps * note_dur
            else:
                st = float(note.time)
